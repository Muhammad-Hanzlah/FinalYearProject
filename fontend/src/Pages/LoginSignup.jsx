...
const LoginSignup = () => {
    ...
    const forgotPassword = async () => {
        let responseData;
        await fetch('https://fluttering-christiana-muhammadhanzalah-eb04cdbe.koyeb.app/forgot-password', {
            method: 'POST',
            headers: { Accept: 'application/json', 'Content-Type': 'application/json' },
    ...
    const resetPassword = async () => {
        let responseData;
        await fetch('https://fluttering-christiana-muhammadhanzalah-eb04cdbe.koyeb.app/reset-password', {
            method: 'POST',
            headers: { Accept: 'application/json', 'Content-Type': 'application/json' },
            body: JSON.stringify({ email: formData.email, otp: otp, newPassword: formData.newPassword }),
        }).then((res) => res.json()).then((data) => responseData = data);
            ...
            <div className="loginsignup-container">
                {isVerifying ? (
                    <>
                        ...
                        <div className="loginsignup-fields">
                            <p className="otp-text">Enter code sent to <b>{formData.email}</b></p>
                            <input type="text" placeholder='Enter OTP' value={otp} onChange={(e) => setOtp(e.target.value)} maxLength="6" className="otp-input" />
                            {state === "Forgot" && (
                        ...
                        <p className="loginsignup-login">
                            Didn't receive code? {timer > 0 ? <span>Wait {timer}s</span> : <span onClick={state === "Forgot" ? forgotPassword : signup}>Resend Code</span>}
                        </p>
                    </>
                ) : (
                    <>
                        ...
                        <div className="loginsignup-fields">
                            {state === "Sign Up" && <input name='username' value={formData.username} onChange={changeHandler} type="text" placeholder='Your Name' />}
                            <input name='email' value={formData.email} onChange={changeHandler} type="email" placeholder='Email Address' />
                            {state !== "Forgot" && <input name='password' value={formData.password} onChange={changeHandler} type="password" placeholder='Password' />}
                        </div>
                        <button onClick={() => {
                        ...
                        <div className="login-options">
                            {state === "Login" && (
                                <p className="forgot-link" onClick={() => setState("Forgot")}>Forgot Password?</p>
                            )}
                            {state === "Sign Up" ? (
                                ...
                                <p className="loginsignup-login">
                                    ...
                                    <span onClick={() => setState(state === "Forgot" ? "Login" : "Sign Up")}>Click Here</span>
                                </p>
                            )}
                        </div>
                    </>
